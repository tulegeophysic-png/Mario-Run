...
function loadLevel(index) {
  ...
  const outsideMushroomCount = Math.max(0, mushroomLimit - springMushrooms.length);
  mushrooms = mushrooms.filter((mushroom) => !mushroom.springSource).slice(0, outsideMushroomCount);
  mushrooms.push(...springMushrooms);
...
function update(delta) {
  ...
  const maxSpeed = player.greenBoost ? 0.39 : (player.big ? 0.252 : 0.24);
  ...
  springs.forEach((spring) => { const item = { x: spring.x - 14, y: spring.y - 16, w: 28, h: 18 }; if (overlap(player, item) && player.vy >= 0) { player.y = spring.y - player.h - 2; player.vy = -0.648; player.grounded = false; spring.bouncing = true; } });
...
function jump(power = -0.72) { if (player.grounded && !gameOver) { const direction = (heldKeys.has('ArrowRight') ? 1 : 0) - (heldKeys.has('ArrowLeft') ? 1 : 0); if (direction) { player.face = direction; player.vx = (player.greenBoost ? 0.39 : (player.big ? 0.252 : 0.24)) * direction; } player.vy = power; player.grounded = false; } }
function highJump() { if (highJumpsRemaining > 0 && player.grounded && !gameOver) { highJumpsRemaining -= 1; jump(-1.05); updateHud(); } }
function runJump() { if (runJumpsRemaining > 0 && player.grounded && !gameOver) { runJumpsRemaining -= 1; player.vx = (player.greenBoost ? 0.675 : (player.big ? 0.585 : 0.465)) * player.face; jump(-0.95); updateHud(); } }
