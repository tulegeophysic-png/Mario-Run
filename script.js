...
function update(delta) {
  ...
  const horizontalAcceleration = (player.greenBoost ? 0.009 : 0.007) * delta;
  ...
  const maxSpeed = player.greenBoost ? 0.31 : (player.big ? 0.25 : 0.21);
  ...
  springs.forEach((spring) => { const item = { x: spring.x - 14, y: spring.y - 16, w: 28, h: 18 }; if (overlap(player, item) && player.vy >= 0) { player.y = spring.y - player.h - 2; player.vy = -0.72; player.grounded = false; spring.bouncing = true; } });
...
function jump(power = -0.36) { if (player.grounded && !gameOver) { player.vy = power; player.grounded = false; } }
function highJump() { if (highJumpsRemaining > 0 && player.grounded && !gameOver) { highJumpsRemaining -= 1; jump(-0.525); updateHud(); } }
function runJump() { if (runJumpsRemaining > 0 && player.grounded && !gameOver) { runJumpsRemaining -= 1; player.vx = (player.greenBoost ? 0.45 : (player.big ? 0.39 : 0.31)) * player.face; jump(-0.475); updateHud(); } }
